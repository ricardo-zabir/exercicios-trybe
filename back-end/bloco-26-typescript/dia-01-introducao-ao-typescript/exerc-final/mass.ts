...
const metrics: string[] = ['kg', 'hg', 'dag', 'g', 'dg', 'cg', 'mg'];
...
const covert = (valor: number, base_unit: string, conversion_unit: string): number => {
    ...
    const valueInGrams: number = values[metrics.indexOf(base_unit)] * valor;
    return valueInGrams / values[metrics.indexOf(conversion_unit)];
}
