...
const metrics: string[] = ['km', 'hm', 'dam', 'm', 'dm', 'cm', 'mm'];
...
const covert = (valor: number, base_unit: string, conversion_unit: string): number => {
    ...
    const valueInMeters: number = values[metrics.indexOf(base_unit)] * valor;
    return valueInMeters / values[metrics.indexOf(conversion_unit)];
}
